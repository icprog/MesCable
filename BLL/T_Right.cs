...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Right dal=DataAccess.CreateT_Right();
		public T_Right()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Right GetModelByCache(int RightID)
		{
			
			string CacheKey = "T_RightModel-" + RightID;
			...
			return (MesWeb.Model.T_Right)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Right> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Right> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Right> modelList = new List<MesWeb.Model.T_Right>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Right model;
