...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Flow_Info dal=DataAccess.CreateT_Flow_Info();
		public T_Flow_Info()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Flow_Info GetModelByCache(int FlowID)
		{
			
			string CacheKey = "T_Flow_InfoModel-" + FlowID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(FlowID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_Flow_Info)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Flow_Info> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Flow_Info> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Flow_Info> modelList = new List<MesWeb.Model.T_Flow_Info>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Flow_Info model;
