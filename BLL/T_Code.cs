...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Code dal=DataAccess.CreateT_Code();
		public T_Code()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Code GetModelByCache(int CodeID)
		{
			
			string CacheKey = "T_CodeModel-" + CodeID;
			...
			return (MesWeb.Model.T_Code)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Code> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Code> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Code> modelList = new List<MesWeb.Model.T_Code>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Code model;
