...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_MaterialType dal=DataAccess.CreateT_MaterialType();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_MaterialType GetModelByCache(int MaterialTypeID)
		{
			
			string CacheKey = "T_MaterialTypeModel-" + MaterialTypeID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(MaterialTypeID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_MaterialType)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_MaterialType> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_MaterialType> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_MaterialType> modelList = new List<MesWeb.Model.T_MaterialType>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_MaterialType model;
