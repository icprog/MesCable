...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_SensorModule dal=DataAccess.CreateT_SensorModule();
		...
		{
			return dal.Exists(SensorModuleID);
		}
		...
		public bool Delete(int SensorModuleID)
		{
			...
			return dal.Delete(SensorModuleID);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		{
			return dal.DeleteList(SensorModuleIDlist );
		}
		...
		public MesWeb.Model.T_SensorModule GetModel(int SensorModuleID)
		{
			...
			return dal.GetModel(SensorModuleID);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_SensorModule GetModelByCache(int SensorModuleID)
		{
			
			string CacheKey = "T_SensorModuleModel-" + SensorModuleID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(SensorModuleID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_SensorModule)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_SensorModule> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_SensorModule> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_SensorModule> modelList = new List<MesWeb.Model.T_SensorModule>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_SensorModule model;
