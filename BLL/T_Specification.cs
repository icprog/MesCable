...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Specification dal=DataAccess.CreateT_Specification();
		...
		public bool Delete(int SpecificationID)
		{
			...
			return dal.Delete(SpecificationID);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		{
			return dal.DeleteList(SpecificationIDlist );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Specification GetModelByCache(int SpecificationID)
		{
			
			string CacheKey = "T_SpecificationModel-" + SpecificationID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(SpecificationID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_Specification)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Specification> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Specification> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Specification> modelList = new List<MesWeb.Model.T_Specification>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Specification model;
