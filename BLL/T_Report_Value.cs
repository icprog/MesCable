...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Report_Value dal=DataAccess.CreateT_Report_Value();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Report_Value GetModelByCache(int Id)
		{
			
			string CacheKey = "T_Report_ValueModel-" + Id;
			...
			return (MesWeb.Model.T_Report_Value)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Report_Value> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Report_Value> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Report_Value> modelList = new List<MesWeb.Model.T_Report_Value>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Report_Value model;
