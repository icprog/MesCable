...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_PARCODE dal=DataAccess.CreateT_PARCODE();
		public T_PARCODE()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_PARCODE GetModelByCache(int ParameterCodeID)
		{
			
			string CacheKey = "T_PARCODEModel-" + ParameterCodeID;
			...
			return (MesWeb.Model.T_PARCODE)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_PARCODE> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_PARCODE> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_PARCODE> modelList = new List<MesWeb.Model.T_PARCODE>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_PARCODE model;
