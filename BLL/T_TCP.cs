...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_TCP dal=DataAccess.CreateT_TCP();
		public T_TCP()
		{}
		#region  BasicMethod
		...
		{
			return dal.DeleteList(TCPIDlist );
		}
		...
		public MesWeb.Model.T_TCP GetModel(int TCPID)
		{
			...
			return dal.GetModel(TCPID);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_TCP GetModelByCache(int TCPID)
		{
			
			string CacheKey = "T_TCPModel-" + TCPID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(TCPID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_TCP)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_TCP> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_TCP> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_TCP> modelList = new List<MesWeb.Model.T_TCP>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_TCP model;
