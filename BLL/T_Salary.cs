...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Salary dal=DataAccess.CreateT_Salary();
		public T_Salary()
		{}
		#region  BasicMethod
		...
		{
			return dal.DeleteList(SalaryIDlist );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Salary GetModelByCache(int SalaryID)
		{
			
			string CacheKey = "T_SalaryModel-" + SalaryID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(SalaryID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_Salary)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Salary> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Salary> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Salary> modelList = new List<MesWeb.Model.T_Salary>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Salary model;
