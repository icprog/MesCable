...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_CodeUsing dal=DataAccess.CreateT_CodeUsing();
		public T_CodeUsing()
		{}
		#region  BasicMethod
		...
		{
			return dal.DeleteList(CodeIDlist );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_CodeUsing GetModelByCache(int CodeID)
		{
			
			string CacheKey = "T_CodeUsingModel-" + CodeID;
			object objModel = MES.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(CodeID);
					if (objModel != null)
					{
			...
			return (MesWeb.Model.T_CodeUsing)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_CodeUsing> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_CodeUsing> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_CodeUsing> modelList = new List<MesWeb.Model.T_CodeUsing>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_CodeUsing model;
