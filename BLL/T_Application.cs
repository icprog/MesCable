...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Application dal=DataAccess.CreateT_Application();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Application GetModelByCache(int ApplicationID)
		{
			
			string CacheKey = "T_ApplicationModel-" + ApplicationID;
			...
			return (MesWeb.Model.T_Application)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Application> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Application> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Application> modelList = new List<MesWeb.Model.T_Application>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Application model;
