...
namespace MesWeb.BLL
{
	...
	{
		private readonly IT_Flow dal=DataAccess.CreateT_Flow();
		public T_Flow()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MesWeb.Model.T_Flow GetModelByCache(int FlowID)
		{
			
			string CacheKey = "T_FlowModel-" + FlowID;
			...
			return (MesWeb.Model.T_Flow)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Flow> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MesWeb.Model.T_Flow> DataTableToList(DataTable dt)
		{
			List<MesWeb.Model.T_Flow> modelList = new List<MesWeb.Model.T_Flow>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MesWeb.Model.T_Flow model;
